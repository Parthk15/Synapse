...
export function Header() {
  ...
  return (
    <>
          ...
          {user && (
            <div className="flex items-center space-x-3">
              <button
                ...
                className="inline-flex items-center gap-2 rounded-lg border border-indigo-500/30 bg-indigo-950/40 px-3.5 py-1.5 text-xs font-medium text-indigo-300 hover:bg-indigo-900/50 hover:border-indigo-500/50 transition-all shadow-md"
              >
                <span>🔍</span>
      ...
      {/* Import & render Semantic Search Modal dynamically */}
      {searchOpen && (
        <SemanticSearchModal isOpen={searchOpen} onClose={() => setSearchOpen(false)} />
      )}
    </>
  );
}
